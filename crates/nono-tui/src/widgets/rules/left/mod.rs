...
impl RowRulesWidget {
    ...
    fn draw(&self, area: Rect, buf: &mut Buffer, state: &AppState) {
        ...
        for row in vp.row_start..vp.row_end {
            ...
            let run_area = Rect { x, y, ..area };
            self.draw_runs(&info, Alignment::Right, run_area, buf, state);

            if cursor.y == row && !matches!(state.focus, Focus::RulesTop) {
                self.draw_runs(&info, Alignment::Left, rule_state.overflow_area, buf, state);
            }
    ...
    fn draw_runs(
        ...
        for (r, run) in runs.iter().enumerate() {
            ...
            tracing::debug!("\tRun {r:?} has len {len} (current width {width:?}, run {run:?})");
            ...
            // Instead hide the remaining runs
                ...
                let style = run_style(info, run.fill, r as u16, state);
